...
public class GravityBall extends GameObject {
    ...
    public GravityBall(ImageView imageView, float x, float y, int width, int height, float radius) {
    ...
    public void handleRocketCollision(float tilt, float rocketZAcceleration) {
        float newVx = (float) (vy*Math.sin(2*tilt) + vx*Math.cos(2*tilt));
        float newVy = (float) (-vy*Math.cos(2*tilt) - vx*Math.sin(2*tilt));
        vx = newVx;
        vy = newVy;
        ay = Config.g * Config.pixelsPerMeter + rocketZAcceleration;
    }
